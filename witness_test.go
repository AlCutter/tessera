...
func TestPopulatePolicy(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			if _, err := populatePolicy(&pol, test.group); err != nil {
				t.Fatalf("failed to populate policy: %v", err)
			}
			...
			// Verify that every group in pol.Groups has valid member names
...
`, wit1CoSigVKey),
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			pol, err := group.toPolicy()
			if err != nil {
				t.Fatalf("group.toPolicy() error = %v", err)
			}
			for _, sTest := range test.satisfyTests {
				signedNote := signNote(t, sTest.signers...)
				got := pol.Satisfied(signedNote)
...
func TestWitnessGroup_Satisfied(t *testing.T) {
	...
	wit1, _ := NewWitness(testWit1VKey, bastion.JoinPath("wit1prefix"))
	wit2, _ := NewWitness(testWit2VKey, bastion.JoinPath("wit2prefix"))
	wit3, _ := NewWitness(testWit3VKey, directURL)
	wit1Sign, _ := f_note.NewSignerForCosignatureV1(testWit1SKey)
	wit2Sign, _ := f_note.NewSignerForCosignatureV1(testWit2SKey)
	wit3Sign, _ := f_note.NewSignerForCosignatureV1(testWit3SKey)
	...
	for _, test := range []struct {
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			cp, err := note.Sign(n, test.signers...)
			if err != nil {
				t.Fatal(err)
			}
			if got, want := test.group.Satisfied(cp), test.expectSatisfied; got != want {
...
func TestWitnessGroup_URLs(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			for u := range test.group.WitnessEndpoints() {
				gotURLs = append(gotURLs, strings.TrimRight(u, "/"))
			}
			slices.Sort(gotURLs)
			slices.Sort(test.expectedURLs)

			if !slices.Equal(gotURLs, test.expectedURLs) {
				t.Errorf("Expected %s but got %s", test.expectedURLs, gotURLs)
			}
		})
	}
}
...
func TestWitnessGroup_toPolicy_StructLiteral(t *testing.T) {
	wit1URL, _ := url.Parse("https://witness.example.com")
	wit1, _ := NewWitness(testWit1VKey, wit1URL)
	wit1Sign, _ := f_note.NewSignerForCosignatureV1(testWit1SKey)

	// Construct WitnessGroup directly as a struct literal (without NewWitnessGroup)
	wg := WitnessGroup{
		N:          1,
		Components: []policyComponent{wit1},
	}
	pol, err := wg.toPolicy()
	if err != nil {
		t.Fatalf("wg.toPolicy() error = %v", err)
	}
	if pol.Quorum == "" || pol.Quorum == "none" {
		t.Fatalf("expected non-empty quorum, got %q", pol.Quorum)
	}
	t.Logf("Pol:\n%v", pol)
	// Verify that the quorum group exists in pol.Groups
	found := false
	for _, g := range pol.Groups {
		if g.Name == pol.Quorum {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("quorum %q not found in pol.Groups: %+v", pol.Quorum, pol.Groups)
	}

	signedNote := signNote(t, wit1Sign)
	if !pol.Satisfied(signedNote) {
		t.Errorf("expected policy to be satisfied for struct-literal WitnessGroup")
	}
	...
	nestedPol, err := nested.toPolicy()
	if err != nil {
		t.Fatalf("nested.toPolicy() error = %v", err)
	}
	if !nestedPol.Satisfied(signedNote) {
		t.Errorf("expected nested struct-literal policy to be satisfied")
	}
}

func TestWitnessGroup_toPolicy_WitnessNameCollision(t *testing.T) {
	// Create two witnesses with the same key name but different keys
	sKey1, vKey1, err := note.GenerateKey(nil, "shared-name")
	if err != nil {
		t.Fatalf("GenerateKey: %v", err)
	}
	sKey2, vKey2, err := note.GenerateKey(nil, "shared-name")
	if err != nil {
		t.Fatalf("GenerateKey: %v", err)
	}
	...
	if len(pol.Witnesses) != 2 {
		t.Fatalf("expected 2 witnesses in policy, got %d: %+v", len(pol.Witnesses), pol.Witnesses)
	}
	if pol.Witnesses[0].Name == pol.Witnesses[1].Name {
		t.Errorf("expected distinct witness names, got %q for both", pol.Witnesses[0].Name)
	}

	sign1, err := f_note.NewSignerForCosignatureV1(sKey1)
	if err != nil {
		t.Fatalf("NewSignerForCosignatureV1: %v", err)
	}
	sign2, err := f_note.NewSignerForCosignatureV1(sKey2)
	if err != nil {
		t.Fatalf("NewSignerForCosignatureV1: %v", err)
	}

	// 1 signature should not satisfy threshold of 2
	sig1Only := signNote(t, sign1)
	if pol.Satisfied(sig1Only) {
		t.Errorf("policy should not be satisfied with only 1 signature")
	}

	// Both signatures should satisfy
	bothSigs := signNote(t, sign1, sign2)
	if !pol.Satisfied(bothSigs) {
		t.Errorf("policy should be satisfied with both signatures")
	}
}

func TestNewWitnessGroupFromPolicy_MLDSA(t *testing.T) {
	_, vKey, err := f_note.GenerateMLDSAKey("oid/1.3.6.1.4.1.32473.106")
	if err != nil {
		t.Fatalf("GenerateMLDSAKey: %v", err)
	}

	polText := fmt.Sprintf("witness w1 %s https://wit1.example.com\nquorum w1\n", vKey)
	wg, err := NewWitnessGroupFromPolicy([]byte(polText))
	if err != nil {
		t.Fatalf("NewWitnessGroupFromPolicy failed for ML-DSA key: %v", err)
	}
	if wg.N != 1 {
		t.Errorf("wg.N = %d, want 1", wg.N)
	}
}
